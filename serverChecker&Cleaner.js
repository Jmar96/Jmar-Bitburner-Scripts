      ...
      if (serv === "home") {
        ...
        if (ns.getServerRequiredHackingLevel(serv) < ns.getHackingLevel()) {
          if (threadCountPerTask > 0) {
            // ns.tprint("Server: " + serv + ", threadCount: " + threadCount + ", threadCountPerTask: " + threadCountPerTask);
            for (let taskName of taskNames) {
              ...
              if (p >= portsRequired) {
                ...
                if (!ns.fileExists(taskName, serv)) {
                  ns.tprint("INFO::NotExist (will copy and run the script[" + taskName + "]) | Threads available: " + threadCount + ", Threads used: " + threadCountPerTask + " | " + serv);
                } else {
                  ns.rm(taskName, serv);
                  ns.tprint("ERROR::Script Exist (Delete Script) | Threads available: " + threadCount + " | " + serv);
                }
              } else {
                ns.rm(taskName, serv);
                ns.tprint("ERROR::Ports opened [" + p + "] vs Ports required [" + portsRequired + "])| (Delete Script) | Threads available: " + threadCount + " | " + serv);
              }
              await ns.sleep(700);
            }
            ...
            if (p >= portsRequired) {
              ...
              if (!ns.fileExists(firstTaskName, serv)) {
                ns.tprint("INFO::NotExist (will copy and run the script[" + firstTaskName + "]) | Threads available: " + threadCount + ", Threads used: " + threadCount + " | " + serv);
              } else {
                ns.rm(firstTaskName, serv);
                ns.tprint("ERROR::Script Exist (Delete Script) | Threads available: " + threadCount + " | " + serv);
              }
            } else {
              ns.rm(firstTaskName, serv);
            ...
            ns.tprint("ERROR::[O]Threads available | Threads available: " + threadCount + " | " + serv);
          }
        } else {
          ns.tprint("WARN::Required hacking level not met.");
        }
      }
      await ns.sleep(700);
    }
